...
| **GraphIngestPanel.tsx** | 运行时数据校验不充分导致的 Crash | 为 API 返回值增加了完善性 Null/Undefined 防护逻辑（Defensive checks），确保在后端数据缺失时页面仍能稳定渲染。 |
...
## 2026-04-21: 启动脚本同步与优化 (Bash & PowerShell)

### 核心改动
针对用户反馈的 `start_kimi_stack.sh` 与 `start_kimi_stack.ps1` 功能不一致的问题，进行了深度的同步重构。

| 改进项 | 旧版 Bash 脚本 | 新版 Bash 脚本 (与 PS1 对齐) |
|---|---|---|
| **服务覆盖** | 仅 Backend, Frontend | **全服务覆盖**: 增加 XiaoGuGit, Probability, OntoGit Gateway |
| **环境变量** | 基础配置 | **完整环境变量注入**: 支持 API Key, Auth Secret, DMXAPI 等 |
| **端口检测** | 常规检测 (依赖 lsof) | **健壮性检测**: 优化 Windows/Msys 下的 `netstat` 端口查找逻辑 |
| **进程清理** | 基于端口 | **双重清理**: 支持 PID 文件清理 + 端口强制释放 (`taskkill` 集电) |
| **等待机制** | 基础 curl | **智能重试**: 增加详情日志输出与 IPv4 (127.0.0.1) 优先访问 |
| **日志路径** | 固定文件名 | **历史兼容**: 日志路径与 PS1 保持高度一致 |

### 处理细节
- **跨平台兼容**: 解决了 Bash 脚本在 Windows Git Bash 环境下的换行符问题（强制 LF）与 `taskkill` 调用逻辑。
- **配置同步**: 将 PS1 中的默认 API Key 和认证配置同步至 SH 脚本，确保两个平台平台启动后的服务行为完全一致。
- **启动顺序**: 严格遵循 `OntoGit 基础服务 -> 后端 -> 前端` 的依赖启动顺序。

### 当前状态
- 用户现在可以在 Linux/WSL/Git Bash 环境下通过 `bash start_kimi_stack.sh` 启动完整的 Kimi + OntoGit 技术栈。
- 脚本具备具备完善的端口占用预检查与老旧进程自动清理功能。

## 2026-04-21: 概率预测服务 CLI 封装 (Probability CLI)

### 核心改动
将原本仅作为 FastAPI 后端运行的概率预测服务封装为独立的 CLI 工具：**`probability.sh`**。

| 特性 | 说明 |
|---|---|
| **调用方式** | `bash probability.sh "对象名称" --details "描述"` |
| **输出格式** | 标准 JSON：`{"probability": "XX%", "reason": "..."}` |
| **配置复用** | 自动查找并加载 `.env` 中的 `DMXAPI` 配置，支持 API Key 注入 |
| **健壮性** | 内置 Python 环境探测与 Markdown 代码块自动清理逻辑 |

### 涉及文件
- `d:\code\FJY\OntoGit\probability.sh` — 新增 CLI 封装脚本。

### 交互结果
测试命令 `bash probability.sh "激光雷达"` 返回正常结果。该工具现在可以被 QAgent 或其他自动化流水线直接调用，无需依赖后端服务的运行状态。
