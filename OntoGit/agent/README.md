...
如果你是从上层 `qagent` 调用，建议优先把 `agent/run_git_query_agent.py` 作为自然语言总入口来用；当你已经明确知道要调哪一个底层工具时，再回退到 `run_git_tool.py`，这样更稳定也更容易编排。
...
```bash
python run_git_query_agent.py "学校当前社区推荐版本是什么？" \
  --project-id demo \
  --start-gateway
```
...
- 先用 `run_git_query_agent.py` 处理自然语言问题
- 如果返回 `unsupported`，再判断是否需要改写问题或切到 `run_git_tool.py`
- 如果需要精确调用某个底层能力，直接用 `run_git_tool.py` 会更可控
