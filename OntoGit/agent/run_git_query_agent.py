...
DEFAULT_GATEWAY_DIR = Path(__file__).resolve().parent.parent / "gateway"
...
def resolve_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    # Define common dummy placeholders generated by some Agent systems
    DUMMY_VALUES = {
        "YOUR_API_KEY",
        "YOUR_BEARER_TOKEN",
        "https://api.example.com",
        "https://www.dmxapi.cn/v1",  # Avoid generic fallback if env has better URL
    }

    def _is_dummy(val: Any) -> bool:
        return str(val).strip() in DUMMY_VALUES

    # Use arguments if provided and not a dummy placeholder; otherwise fallback to .env or default
    base_url = args.base_url if args.base_url and not _is_dummy(args.base_url) else (env_values.get("GATEWAY_BASE_URL") or "http://127.0.0.1:8080")
...
def _run(args: argparse.Namespace) -> tuple[int, dict[str, object]]:
    ...
    except Exception as exc:  # pragma: no cover - defensive guard for unexpected client init errors
