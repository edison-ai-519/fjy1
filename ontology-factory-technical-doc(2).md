...
- `ablation_judge`：小故判定
  - **方法论**：基于“无之以为用”的消融原理，测量实体对整体名实耦结构的“贡献重度”。
  - **执行流**：
    - 输入：`focus_entity` + `related_relations` + `context_entities`。
    - 并行双路调用：
      - `keep_probability` ($P_k$): 保留该实体时的系统概率。
      - `remove_probability` ($P_r$): 去除该实体后的剩余系统概率。
    - 判定决策：
      - 当 $P_k - P_r \ge 10$ 时，标记 `is_hit = true`，判定该实体为该文档知识体系中的“核心因果节点（小故）”。
      - 自动生成对照性判定依据（如：“保留版：增强了语义联系；去除版：导致逻辑链断裂”）。
...
- `消融`：展示 `ablation` 阶段输出的影响等级、原因和系统风险
...
- 从阶段结果中提取 `entity_count`、`relation_count`、`ablation_count`
- 对数组、对象和字符串做基础类型归一化
