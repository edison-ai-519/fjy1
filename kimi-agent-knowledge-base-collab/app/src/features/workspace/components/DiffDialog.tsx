            ...
            {diffData ? (
              <div className="space-y-1 font-mono text-xs">
                {lines.map((line, index) => (
                  <div key={index} className={`${line.startsWith('+') ? 'text-green-400 bg-green-950/30' : line.startsWith('-') ? 'text-red-400 bg-red-950/30' : 'text-slate-400'}`}>
                    {line}
                  </div>
                ))}
                {lines.length === 0 && <div className="text-slate-500 italic">两个版本之间内容一致。</div>}
              </div>
            ) : (
