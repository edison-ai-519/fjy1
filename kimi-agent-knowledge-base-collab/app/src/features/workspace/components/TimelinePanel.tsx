        ...
        <ScrollArea className="h-full">
          <div className="p-6 space-y-2">
            {selectedFile ? commits.map((commit, index) => (
              <div key={commit.id} className="relative pl-8 pb-6 group">
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-mono font-black text-primary/80 uppercase tracking-tighter">{commit.id.slice(0, 7)}</span>
                    <span className="text-[10px] uppercase font-black tracking-widest text-muted-foreground/40">{new Date(commit.timestamp).toLocaleTimeString()}</span>
                  </div>
                  <p className="text-[13px] leading-relaxed font-bold text-foreground/90">{commit.message}</p>
                  <div className="flex items-center gap-4 mt-1.5">
                    ...
                    <div className="flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                      <button onClick={() => onViewDiff(commit.id)} className="text-[10px] font-black uppercase tracking-widest text-muted-foreground hover:text-foreground flex items-center gap-1.5">
                        <ArrowLeftRight className="h-3 w-3" />
                        Diff
                      </button>
                      <button onClick={() => onSetOfficial(commit.id)} className="text-[10px] font-black uppercase tracking-widest text-amber-500/70 hover:text-amber-500 flex items-center gap-1.5">
                        <ShieldCheck className="h-3 w-3" />
                        Official
                      </button>
                      <button onClick={() => onRollback(commit.id)} className="text-[10px] font-black uppercase tracking-widest text-primary/70 hover:text-primary flex items-center gap-1.5">
                        Rollback
                      </button>
                    </div>
                  </div>
